    ...
    while position.get_play():
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 3:  # Right mouse
                ...
                is_clicked = True
                if (right_x, right_y) not in is_right_clicked:
                    is_right_clicked.append((right_x, right_y))
                else:
                    index = is_right_clicked.index((right_x, right_y))
                    is_right_clicked.pop(index)

            if (is_down or is_clicked) and event.type == pygame.MOUSEBUTTONUP and event.button == 3:
                pos = pygame.mouse.get_pos()
                chess_coord = pixel_coord_to_chess(pos, square_width, square_height)
            ...
            if not is_down and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                is_right_clicked = []
                pos = pygame.mouse.get_pos()
                chess_coord = pixel_coord_to_chess(pos, square_width, square_height)
                x = chess_coord[0]
                ...
                is_down = True  # A piece is being dragged
            ...
            if (is_down or is_clicked) and event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                is_down = False  # Mouse is released
                ...
                if (x, y) == (x2, y2):
                    ...
                    draw_board(screen, background, position, pieces_image,
                               square_width, square_height, images, is_clicked=is_clicked)
        ...
        if not is_down and not is_clicked and not is_transition:
            # Update board after each turn, when transition is done
            draw_board(screen, background, position, pieces_image,
                       square_width, square_height, images,
                       right_clicked=is_right_clicked, is_clicked=is_clicked)
